...
The core library is written in C11 and targets POSIX, Windows, Arduino and Python environments, with no external dependencies; [Zlib](https://zlib.net/) is only required when compression is enabled.<br />Its low footprint makes Nyx suitable for deployment on microcontrollers such as ESP32, Cortex-M, …
