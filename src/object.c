...
buff_t nyx_buffer_ndup(BUFF_t b, size_t n)
{
    if(b != NULL)
    {
        buff_t buff = nyx_memory_alloc(n);
...
void nyx_object_free_recursive(nyx_object_t *object)
{
    /*----------------------------------------------------------------------------------------------------------------*/
    ...
    switch(object->type)
    {
        ...
        case NYX_TYPE_LIST:
        {
            nyx_list_t *list = (nyx_list_t *) object;

            for(nyx_list_node_t *node = list->head, *next; node != NULL;)
            {
                next = node->next;
                nyx_object_free_recursive(node->value);
                nyx_memory_free(node);
                node = next;
            }

            nyx_memory_free(list);
        ...
        case NYX_TYPE_DICT:
        {
            nyx_dict_t *dict = (nyx_dict_t *) object;

            for(nyx_dict_node_t *node = dict->head, *next; node != NULL;)
            {
                next = node->next;
                nyx_object_free_recursive(node->value);
                nyx_memory_free(node);
